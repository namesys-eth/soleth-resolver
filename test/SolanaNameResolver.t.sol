 ...
 */
contract SolanaNameResolverTest is Test {
    using Utils for *;
    using Helper for *;

    string public chainID = block.chainid == 1 ? "1" : "5";
    SolanaNameResolver public SNR = new SolanaNameResolver();
    ...
    function testJsonMap() public {
        assertEq(SNR.funcMap(iResolver.contenthash.selector), "contenthash");
        assertEq(SNR.funcMap(iResolver.addr.selector), "address/eth");
        assertEq(SNR.funcMap(iResolver.pubkey.selector), "publickey");
        assertEq(SNR.funcMap(iResolver.name.selector), "name");
        //assertEq(SNR.funcMap(iResolver.recordVersions.selector), "version");

        assertEq(SNR.jsonFile(abi.encodeWithSelector(iResolver.text.selector, bytes32(0), string("key"))), "text/key");
        assertEq(
            SNR.jsonFile(abi.encodeWithSelector(iOverloadResolver.addr.selector, bytes32(0), uint256(0))), "address/0"
        );
        /*
        assertEq(
            SNR.jsonFile(
        ...
        assertEq(SNR.jsonFile(abi.encodeWithSelector(iResolver.ABI.selector, bytes32(0), uint256(777))), "abi/777");
        assertEq(
            SNR.jsonFile(abi.encodeWithSelector(iResolver.dnsRecord.selector, bytes32(0), bytes32(0), uint16(1234))),
    ...
    function testTransferOwner() public {
        ...
        SNR.transferOwnership(address(0));
        assertEq(SNR.owner(), address(0));
    }
    /*
    function testCoreDomain() public {
        ...
        (bool _core, string memory _gateway, string memory _fallback) = SNR.dev3Space(_node);
        assertEq(_core, true);
        ...
        (_core, _gateway, _fallback) = SNR.dev3Space(_node);
        assertEq(_core, false);
        assertEq(_gateway, "");
        assertEq(_fallback, "");
        SNR.setCoreDomain(_node, "virgil.ethereum.org", "");
        (_core, _gateway, _fallback) = SNR.dev3Space(_node);
        assertEq(_core, true);
        assertEq(_gateway, "virgil.ethereum.org");
        assertEq(_fallback, "");
        SNR.removeCoreDomain(_node);
        (_core, _gateway, _fallback) = SNR.dev3Space(_node);
        assertEq(_core, false);
        assertEq(_gateway, "");
        assertEq(_fallback, "");
    ...
    */
    /**
     * @dev Test case for resolving the domain "domain.eth" using SNR
     */
    /*
    function test1_domaineth() public {
        ...
        SNR.setupYourENS(_node, _approver, "example.com", "example.xyz");
        bytes memory _request = abi.encodeWithSelector(iResolver.addr.selector, _node);
        bytes memory _calldata = abi.encodeWithSelector(SolanaNameResolver.resolve.selector, _encoded, _request);
        bytes32 _callhash = keccak256(_calldata);
        bytes32 _checkhash = keccak256(abi.encodePacked(address(SNR), blockhash(block.number - 1), _callhash));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                SolanaNameResolver.__callback.selector,
                _extradata
            )
        );
        SNR.resolve(_encoded, _request);
        bytes memory _result = abi.encode(address(type(uint160).max));
        bytes32 _approvalDigest = address(SNR).GetApprovalDigest(_signer, "example.com", chainID);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ApproverKey, _approvalDigest);
        bytes memory _approvalSig = abi.encodePacked(r, s, v);
        bytes32 _signerDigest = address(SNR).GetRecordDigest(_signer, "example.com", "address/60", _result, chainID);
        ...
        assertEq(SNR.__callback(_response, _extradata), _result);
    }*/
    /**
     * @dev Test case for resolving the deep sub domain "hello.darkness.my.old.friend.domain.eth" using SNR
     */
    /*
    ...
    */
    /**
     * @dev Test case for resolving "dev3.eth" using SNR
     */
    /*
    function test2_dev3eth() public {
        ...
        SNR.resolve(_encoded, _request);
        bytes memory _result = abi.encode(address(type(uint160).max));
        bytes32 _approvalDigest = address(SNR).GetApprovalDigest(_signer, "namesys-eth.github.io", chainID);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ApproverKey, _approvalDigest);
        bytes memory _approvalSig = abi.encodePacked(r, s, v);
        bytes32 _signerDigest =
            address(SNR).GetRecordDigest(_signer, "namesys-eth.github.io", "address/60", _result, chainID);
        (v, r, s) = vm.sign(SignerKey, _signerDigest);
        bytes memory _recSig = abi.encodePacked(r, s, v);
        SNR.setCoreApprover(dev3Node, _approver, true);
        bytes memory _response =
            abi.encodeWithSelector(iCallbackType.signedRecord.selector, _signer, _recSig, _approvalSig, _result);
        assertEq(SNR.__callback(_response, _extradata), _result);
    }
    */
     ...
     */
    /*
    function testDeepSubSolanaNameResolvereth() public {
        ...
        SNR.resolve(_encoded, _request);
        bytes memory _result = abi.encode(address(type(uint160).max));
        bytes32 _approvalDigest = address(SNR).GetApprovalDigest(_signer, "0xc0de4c0ffee.github.io", chainID);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ApproverKey, _approvalDigest);
        bytes memory _approvalSig = abi.encodePacked(r, s, v);
        bytes32 _signerDigest =
            address(SNR).GetRecordDigest(_signer, "0xc0de4c0ffee.github.io", "address/60", _result, chainID);
        (v, r, s) = vm.sign(SignerKey, _signerDigest);
        bytes memory _recSig = abi.encodePacked(r, s, v);
        SNR.setCoreApprover(dev3Node, _approver, true);
        bytes memory _response =
    ...
    */
    /**
     * @dev Test case for resolving sub.dev3.eth using SNR
     */
    /*
    ...
    function test4_approvalSignature() public {
        ...
        assertEq(_approver, SNR.getSigner(_message, _signature));
    ...
    function test5_signerSignature() public {
        ...
        bytes memory _signature = abi.encodePacked(r, s, v);
        assertEq(_signer, SNR.getSigner(_message, _signature));
        _signature = abi.encodePacked(r, s, uint256(v));
        assertEq(_signer, SNR.getSigner(_message, _signature));
        bytes32 vs = bytes32(uint256(v - 27) << 255) | s;
        _signature = abi.encodePacked(r, vs);
